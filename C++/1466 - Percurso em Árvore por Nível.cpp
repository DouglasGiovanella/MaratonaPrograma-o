...
class BinaryTree{
	...
	///////////////////////////////////////////////////////////////////////////////
	// PATH BY LEVEL
    // Generates the path visiting each level in order
    /// O( V + E )
	public:vector<T> pathByLevel( ){
		vector<T> path;
		queue< BinaryTreeNode<T>* > tosee;
		if( root != LEAF ){
			tosee.push( root );
			BinaryTreeNode<T> *aux;
			while( !tosee.empty() ){
				aux = tosee.front();
				tosee.pop( );
				if( aux == LEAF ) continue;
				path.push_back( aux->getElement() );
				tosee.push( *(aux->getLeftBranch()) );
				tosee.push( *(aux->getRightBranch()) );
			}
		}
		return path;
	}
